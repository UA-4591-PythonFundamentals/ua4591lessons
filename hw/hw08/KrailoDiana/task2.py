...
def check_password (password):
    ...
    if len(password) < 6 or len(password) > 16:
        return "Your password length is wrong, it must be between 6 and 16 characters"
    
    if not re.search("[A-Z]", password):
        return "There are not enough capital letters in the password"
    if not re.search("[a-z]", password):
        return "There are not enough lowercase letters in the password"
    if not re.search("[0-9]", password):
        return "There are not enough numbers in the password"
    if not re.search("[$#@]", password):
        return "There are not enough characters in the password"
    
    return "Your password is super duper strong"
